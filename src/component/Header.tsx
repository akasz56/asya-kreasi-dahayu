...
export default function Header() {
    ...
    return (
        <header className="bg-neutral-900">
            <nav className="mx-auto flex max-w-screen-2xl h-24 items-center justify-between" aria-label="Global">
                <div className="flex lg:flex-1">
                    <Link href={"/"} className='text-xl font-normal leading-[1.5em] tracking-wider'>ASYA KREASI DAHAYU</Link>
                </div>
                ...
                <div className="hidden lg:flex lg:gap-x-12">
                    <ul className='flex space-x-16'>
                        {pages.map((item, key) => (
                            <li key={key}>
                                <Link href={item.href} className='text-xl font-normal leading-[1.5em] tracking-wider'>{item.title}</Link>
                            </li>
                        ))}
                    </ul>
                </div>
                                        ...
                                        <div className="flex h-full flex-col overflow-y-scroll bg-neutral-300 py-6 shadow-xl">
                                            ...
                                            <div className="relative mt-6 flex-1 px-4 sm:px-6">
                                                <div className="flex flex-col">
                                                    {pages.map((item, key) => (
                                                        <Link key={key} href={item.href} className='text-black text-3xl font-medium leading-[3rem] tracking-wider mb-3'>{item.title}</Link>
                                                    ))}
                                                </div>
                                            </div>
                                        </div>
